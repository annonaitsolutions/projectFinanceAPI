					...
					<c:if test="${! empty disputeList}">
						<c:forEach items="${disputeList}" var="master">
							<tr>
								...
								<td><a href="addOrModifyReportView?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.view"/></a></td>
							<td><a href="addOrModifyReportCompare?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.compare"/></a></td>
							 <td><a href="addOrModifyReportAddArbitration?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.arbitration"/></a></td>
							<td><a href="addOrModifyReportCustAccept?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.accept"/></a></td>
							</tr>
						</c:forEach>
					</c:if>
