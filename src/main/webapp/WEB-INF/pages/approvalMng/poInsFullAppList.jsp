					...
					<c:if test="${! empty dispute}">
						<c:forEach items="${dispute}" var="master">
							<tr>
								...
								<td><a href="addOrModifyReportAppView?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.view"/></a></td>
								<td><a href="addOrModifyReportAppCompare?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.compare"/></a></td>
							</tr>
						</c:forEach>
					</c:if>
