					...
					<c:if test="${! empty masterList}">
						<c:forEach items="${masterList}" var="master">
							<tr>
								...
								<td><a href="generalLedgerAppMngView?id=${master.id}"
									class="btn btn-primary"><spring:message code="label.view"/></a></td>
									<td><a href="generalLedgerGraphicalView?id=${master.id}"
									class="btn btn-primary"><spring:message code="label.ghView"/></a></td>
							</tr>
						</c:forEach>
					</c:if>
