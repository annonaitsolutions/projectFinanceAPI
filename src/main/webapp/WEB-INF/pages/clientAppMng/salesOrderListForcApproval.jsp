			...
			<table class="table data jqtable example" id="my-table">
				...
				<tbody>
					<c:if test="${! empty salesOrder}">
						<c:forEach items="${salesOrder}" var="master">
							<tr>
								<td><c:out value="${master.id}"></c:out></td>
								<td><c:out value="${master.customerName}"></c:out>
								<td><c:out value="${master.buyerName}"></c:out>
								<td><c:out value="${master.masterPlanKey}"></c:out></td>
								<td><c:out value="${master.status}"></c:out></td>
								<fmt:formatDate var='masterPlanDate' value='${master.masterPlanDate}' pattern="dd/MM/yyyy" />
								<td><c:out value="${masterPlanDate}"></c:out></td>
								<td><c:out value="${master.amount}"></c:out></td>
								<td><c:out value="${master.goods}"></c:out></td>
								<td><c:out value="${master.quantity}"></c:out></td>
								<td><a href="salesOrderForcApproval?id=${master.id}"class="btn btn-primary"><spring:message code="label.select"/></a></td>
