...
<script>
	...
	function simple() {
		...
		if (document.interest.buyingCostSanc.value == " ") {
			...
			r = parseFloat(document.interest.plrRate.value);
			n = parseFloat(document.interest.tenure.value);
			
			i = ((p * r / 100) / 365) * n;			
			
			document.interest.total1.value = parseFloat(i);			

			document.interest.rateOfInt1.value = parseFloat(i);
	...
	function Amount() {
		var p, f, per, i, amnt, a, b, c, d,f,g, flt, pt, process, doc, late, tax,tax1, amount;
			...
			f = parseInt(document.interest.flatCharges.value);

			per = parseInt(document.interest.percentAmt.value);

			a = parseInt(document.interest.procFee.value);
			b = parseInt(document.interest.docFee.value);
			c = parseInt(document.interest.lateFee.value);
			...
			flt = parseInt(document.interest.flatCharges.value);
			...
			amnt = p + f + i + per + a + b + c + d + f + g;

			document.interest.funalAmt.value = amnt;
	...
	function plr() {
		...
		amt = p + i;
		document.interest.calPlrRate.value = amt;
	...
	function taxamnt1() {
		...
		amt = p * i / 100;
		document.interest.taxAmt1.value = amt;
	}
	...
	function taxamnt2() {
		...
		amt = p * i / 100;
		document.interest.taxAmt2.value = amt;
	}
	...
	function calcInvoice() {
		...
		document.myForm.funalAmt.value = total;
	...
	function calcAmt() {
		...
		document.myForm.totalAmount.value = total;
	...
	function calcAmt1() {
		...
		document.myForm.totalAmount1.value = total;
	...
	function calcAmt2() {
		...
		document.myForm.totalAmount2.value = total;
	...
	function calcAmt3() {
		...
		document.myForm.totalAmount3.value = total;
	...
	function amt1() {
		...
		document.myForm.totalRate.value = total;
	...
	function totalamt1() {
		...
		document.myForm.finalAmt.value = total;
...
<nav class="navbar  navbar2">
	<div class="container-fluid">
	    <div class="navbar-header">
	      <a class="navbar-brand" href="/annona/bnkEmp/bankEmpCommon"><i class="glyphicon glyphicon-home"></i></a>
	          &nbsp;&nbsp;<a href="#" style="color: #005C79;">DHANAPURTHI&nbsp;<sup>TM</sup></a>
	    </div>
		<div class="pull-left">

				<div class="prof_img"><a href="editTradeBankEmpProfile?id=${requestCurrentUser.id}"><span><img src="${requestCurrentUser.imageName}" class="profile_img"></span><span class="customer_name"><spring:message code="label.welcome" /> ${requestCurrentUser.displayName}</span></a></div>

		</div>
	    <div id="myNavbar">
			<ul class="nav navbar-nav navbar-right">
				<li class="log_out"><a href="tBankHelp"><i class="glyphicon glyphicon-modal-window"></i>&nbsp;<strong><spring:message code="label.menuHelp"/></strong></a></li>
