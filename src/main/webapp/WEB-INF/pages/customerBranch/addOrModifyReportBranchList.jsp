					...
					<c:if test="${! empty disputeList}">
						<c:forEach items="${disputeList}" var="master">
							<tr>
								...
								<td><a href="addOrModifyReportBranchView?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.view"/></a></td>
							<td><a href="addOrModifyReportBranchCompare?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.compare"/></a></td>
							</tr>
						</c:forEach>
					</c:if>
