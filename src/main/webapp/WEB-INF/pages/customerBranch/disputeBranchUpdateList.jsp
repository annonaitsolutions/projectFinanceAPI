					...
					<c:if test="${! empty disputeList}">
						<c:forEach items="${disputeList}" var="master">
							<tr>
								...
								<td><a href="disputeBranchUpdate?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.select"/></a></td>
							 <td><a href="addOrModifyReportBranch?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.addReport"/></a></td>
							</tr>
						</c:forEach>
					</c:if>
