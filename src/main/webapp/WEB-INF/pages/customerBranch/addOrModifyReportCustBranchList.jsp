					...
					<c:if test="${! empty disputeList}">
						<c:forEach items="${disputeList}" var="master">
							<tr>
								...
								<td><a href="addOrModifyReportInvoiceBranchAccept?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.accept"/></a></td>
							<td><a href="addOrModifyReportInvoiceBranch?id=${master.id}" class="btn btn-primary" onclick="return confirm();"><spring:message code="label.addReport"/></a></td>
							</tr>
						</c:forEach>
					</c:if>
